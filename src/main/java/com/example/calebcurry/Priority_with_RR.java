...
public class Priority_with_RR {
    public static String PrioritythenRR(ArrayList<Process> q, int time_quantum){
        int count = 0;
        int time = 0;
        String sequence="";
        int minID=-1;
        int y=Utility.greater_than_maximum_value(q,"priority");
        ...
        while(count!=q.size()){
            ...
            if(!waiting.isEmpty()) {
                ...
                if (!rrActive) {
                    minID = Utility.get_live_process_with_minimum_value(waiting, y, 0, "priority");

                    rrActive=Utility.checkForTies(waiting,rrArray,minID,"priority");
                }
